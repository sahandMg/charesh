...
! function(a) {
    ...
    function s(b, c, d) {
        ...
        if (1 === c.results.length && c.teams.length > 1) {
            var h = a('<span class="doubleElimination" style="height: 30px;width: 25px;font-size: 75%;">دو حذفی</span>').appendTo(e);
            h.click(function() {
                if (c.teams.length > 1 && c.results.length < 3) return c.results.push([], []), K(d)
            })
        } else if (3 === c.results.length && c.teams.length > 1) {
            var h = a('<span class="singleElimination" style="width: 25px;height: 40px;font-size: 75%;">تک حذفی</span>').appendTo(e);
            h.click(function() {
